...
public class Map implements Serializable {
	...
	private static final char RAJION = 'R';
	private static final char BIRD = 'V';
	private static final char ENNEMI_BASIQUE = 'E';
	private static final char WALL = '%';
	private static final char BREAKABLE_WALL = '$';
		...
		try {
			...
			while ((ligne=tampon.readLine())!=null) {
				...
				for(int x=0;x<ligne.length();x++) {
					walls[x][y]= ligne.charAt(x) == WALL;
					start_breakable_walls[x][y]= ligne.charAt(x) == BREAKABLE_WALL;

					if (ligne.charAt(x) == ENNEMI_BASIQUE || ligne.charAt(x) == BIRD || ligne.charAt(x) == RAJION) {
						start_agents.add(new InfoAgent(x,y,AgentAction.STOP,ligne.charAt(x),ColorAgent.DEFAULT,false,false));
					}

					if (ligne.charAt(x) == BOMBERMAN) {
						ColorAgent col;
